---
title: URL shortener
description: Design a URL shortening service similar to Bitly and TinyURL
---
...
**Long URL**: [/interviews/url-shortener](/interviews/url-shortener)
...
Contains the new short URL's properties such as `expiration`, `hash`, `originalURL`, and `userID` of the user who created the short URL. We can also use the `hash` column as an [index](/databases/indexes) to improve the query performance.
...
<Info>
For more details, refer to [SQL vs NoSQL](/databases/sql-vs-nosql).
</Info>
...
Now, let's talk about [caching](/fundamentals/caching). As per our estimations, we will require around ~35 GB of memory per day to cache 20% of the incoming requests to our services. For this use case, we can use [Redis](https://redis.io) or [Memcached](https://memcached.org) servers alongside our API server.
...
Now let us talk about [caching](/fundamentals/caching).
...
We can also use an [API Gateway](/architecture/api-gateway) as they can support capabilities like authorization, rate limiting, and load balancing out of the box.
