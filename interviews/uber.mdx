---
title: Uber
description: Design a ride-hailing service similar to Uber, Lyft, and OLA Cabs
---
...
<Accordion title="Estimation and constraints">
...
We will be using [microservices architecture](/architecture/monoliths-microservices) since it will make it easier to horizontally scale and decouple our services. Each service will have ownership of its own data model. Let's try to divide our system into some core services.
...
Since our architecture is microservices-based, services will be communicating with each other as well. Generally, REST or HTTP performs well but we can further improve the performance using [gRPC](/architecture/rest-graphql-grpc#grpc) which is more lightweight and efficient.

[Service discovery](/advanced/service-discovery) is another thing we will have to take into account. We can also use a service mesh that enables managed, observable, and secure communication between individual services.

<Info>
Learn more about [REST, GraphQL, gRPC](/architecture/rest-graphql-grpc) and how they compare with each other.
</Info>
...
Customer requests a ride by specifying the source, destination, cab type, payment method, etc.
</Step>

<Step title="Find nearby drivers">
Ride service registers this request, finds nearby drivers, and calculates the estimated time of arrival (ETA).
</Step>

<Step title="Broadcast to drivers">
...
<Step title="Rate the trip">
After the payment is complete, the customer can leave a rating and feedback for the trip if they like.
</Step>
...
The client can periodically send an HTTP request to servers to report its current location and receive ETA and pricing information. This can be achieved via something like [Long polling](/architecture/long-polling-websockets-sse).

**Push model**

The client opens a long-lived connection with the server and once new data is available it will be pushed to the client. We can use [WebSockets](/architecture/long-polling-websockets-sse) or [Server-Sent Events (SSE)](/architecture/long-polling-websockets-sse) for this.

The pull model approach is not scalable as it will create unnecessary request overhead on our servers and most of the time the response will be empty, thus wasting our resources. To minimize latency, using the push model with [WebSockets](/architecture/long-polling-websockets-sse) is a better choice because then we can push data to the client once it's available without any delay, given that the connection is open with the client. Also, WebSockets provide full-duplex communication, unlike [Server-Sent Events (SSE)](/architecture/long-polling-websockets-sse) which are only unidirectional.
...
<Info>
Learn more about [Long polling, WebSockets, Server-Sent Events (SSE)](/architecture/long-polling-websockets-sse).
</Info>
...
[Geohashing](/advanced/geohashing-quadtrees) is a [geocoding](https://en.wikipedia.org/wiki/Address_geocoding) method used to encode geographic coordinates such as latitude and longitude into short alphanumeric strings. It was created by [Gustavo Niemeyer](https://twitter.com/gniemeyer) in 2008.
...
A [Quadtree](/advanced/geohashing-quadtrees) is a tree data structure in which each internal node has exactly four children. They are often used to partition a two-dimensional space by recursively subdividing it into four quadrants or regions. Each child or leaf node stores spatial information. Quadtrees are the two-dimensional analog of [Octrees](https://en.wikipedia.org/wiki/Octree) which are used to partition three-dimensional space.
...
<Info>
For more details, refer to [Transactions](/databases/transactions) and [Distributed Transactions](/databases/distributed-transactions).
</Info>
...
<Info>
For more details, refer to the [WhatsApp](/interviews/whatsapp) system design where we discuss push notifications in detail.
</Info>
...
To scale out our databases we will need to partition our data. Horizontal partitioning (aka [Sharding](/databases/sharding)) can be a good first step. We can shard our database either based on existing [partition schemes](/databases/sharding#partitioning-criteria) or regions. If we divide the locations into regions using let's say zip codes, we can effectively store all the data in a given region on a fixed node. But this can still cause uneven data and load distribution, we can solve this using [Consistent hashing](/databases/consistent-hashing).

<Info>
For more details, refer to [Sharding](/databases/sharding) and [Consistent Hashing](/databases/consistent-hashing).
</Info>
...
<Info>
For more details, refer to [Caching](/fundamentals/caching).
</Info>
...
- Introducing [load balancers](/fundamentals/load-balancing) between clients, servers, databases, and cache servers.
- Using multiple read replicas for our databases.
- Multiple instances and replicas for our distributed cache.
- Exactly once delivery and message ordering is challenging in a distributed system, we can use a dedicated [message broker](/architecture/message-brokers) such as [Apache Kafka](https://kafka.apache.org) or [NATS](https://nats.io) to make our notification system more robust.
