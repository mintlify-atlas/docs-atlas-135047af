---
title: WhatsApp
description: Design an instant messaging service similar to WhatsApp, Facebook Messenger, and WeChat
---
...
The client can periodically send an HTTP request to servers to check if there are any new messages. This can be achieved via something like [Long polling](/architecture/long-polling-websockets-sse).
...
If the recipient is not active, the chat service will add an event to a [message queue](/architecture/message-queues) with additional metadata such as the client's device platform which will be used to route the notification to the correct platform later on.
...
But where can we store files at scale? Well, [object storage](/fundamentals/storage) is what we're looking for. Object stores break data files up into pieces called objects. It then stores those objects in a single repository, which can be spread out across multiple networked systems. We can also use distributed file storage such as [HDFS](/fundamentals/storage) or [GlusterFS](https://www.gluster.org).
